...
mySocket.onmessage = function (event) {
	...
	globalPassword = window.prompt("Enter your password", "");
	var reply = Aes.Ctr.decrypt(receivable.content, globalPassword, 128) + "1";
	var sendable = { type: "loginClient",
			 content: Aes.Ctr.encrypt(reply, globalPassword, 128) };
	mySocket.send(JSON.stringify(sendable));
    }
    if(receivable.type == "invoiceData") {
	var invoiceData = JSON.parse(Aes.Ctr.decrypt(receivable.content, globalPassword, 128));
	...
	var pdfData = atob(Aes.Ctr.decrypt(receivable.content, globalPassword, 128));
...
function getPreviewPdf(s) {
    ...
    var encryptedSendable = Aes.Ctr.encrypt(JSON.stringify(clientSendable), globalPassword, 128);
